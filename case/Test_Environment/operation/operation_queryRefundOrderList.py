...
@author: Four
Project: operation_queryRefundOrderList.py
URL:http://operation-pc-qa.eslink.net.cn/etbc/order/queryRefundOrderList
Description:云商城-查询退货记录
"""
...
Sheet_Name = "operation_queryRefundOrderList"
logger_message = Logger()


def operation_queryRefundOrderList_1(self):
    u"""查询退货记录"""
    try:
        ...
        queryRefundOrderList_data = {
            "orderCode": "%s" % get_queryMemberOrderList
        }
        data = ExcelUtil(report_xlsx, Sheet_Name).dict_data()
        test_id = 0
        s = requests.session()
        res = send_requests(s, data[test_id], send_load=queryRefundOrderList_data, cookie=self.token)
        return res["result"]
...
class operation_queryRefundOrderList(unittest.TestCase):
        ...
        def test_operation_queryRefundOrderList(self):
            operation_queryRefundOrderList_1(self)
