...
@author: Four
Project: operation_queryLogisticsOrderList.py
URL:http://operation-pc-qa.eslink.net.cn/etbc/order/queryLogisticsOrderList
Description:云商城-查询发货记录
"""
...
Sheet_Name = "queryLogisticsOrderList"
logger_message = Logger()


def operation_queryLogisticsOrderList_1(self):
    u"""查询发货记录"""
    try:
        ...
        queryLogisticsOrderList_data = {
            "orderCode": "%s" % get_queryMemberOrderList
        }
        data = ExcelUtil(report_xlsx, Sheet_Name).dict_data()
        test_id = 0
        s = requests.session()
        res = send_requests(s, data[test_id], send_load=queryLogisticsOrderList_data, cookie=self.token)
        return res["result"]
...
class operation_queryLogisticsOrderList(unittest.TestCase):
        ...
        def test_operation_queryLogisticsOrderList(self):
            operation_queryLogisticsOrderList_1(self)
