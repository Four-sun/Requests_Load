...
@author: Four
Project: operation_queryNewsIndexList.py
URL:http://operation-pc-qa.eslink.net.cn/etbc/product/queryNewsIndexList
Description:云商城-查询店铺首页广告列表
"""
...
Sheet_Name = "operation_queryNewsIndexList"
logger_message = Logger()
send_time = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))


def operation_queryNewsIndexList_1(self):
    u"""查询店铺首页广告列表"""
    try:
...
class operation_queryNewsIndexList(unittest.TestCase):
        ...
        def test_operation_queryAdIndexList(self):
            operation_queryNewsIndexList_1(self)
