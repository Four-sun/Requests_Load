...
@author: Four
Project: case\member_queryMemberCardCombox.py
URL: http://patrol-mobile-qa.eslink.net.cn/member/member/queryMemberCardCombox
Description:会员管理卡关系信息查询
"""
...
Sheet_Name = "member_queryMemberCardCombox"
logger_message = Logger()
send_time = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))


class member_queryMemberCardCombox(unittest.TestCase):
    ...
    def test_queryMemberCardCombox_1(self):
        u"""卡关系信息查询，正确的条件"""
        try:
    ...
    def test_queryMemberCardCombox_2(self):
        u"""卡关系信息查询，正确用户名错误用户电话"""
        try:
            data = ExcelUtil(reportxlsx, Sheet_Name).dict_data()
            test_id = 1
            s = requests.session()
