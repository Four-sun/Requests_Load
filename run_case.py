...
Environment_path = os.path.join(cur_path, "case\\Test_Environment")
send_time = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))


def add_case(caseName="Member", rule="member*.py"):
    """第一步：加载所有的测试用例"""
    case_path = os.path.join(Environment_path, caseName)  # 用例文件夹
...
def send_mail(sender, psw, receiver, smtpserver, report_file, port):
    ...
    att["Content-Disposition"] = 'attachment; filename= "%s-report.html"' % send_time
    msg.attach(att)
    try:
