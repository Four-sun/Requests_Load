...
report_path = os.path.join(cur_path, "logs"+"\\")


class Logger:
    log_fmt = '%(asctime)-8s\tline%(lineno)-8s\t%(levelname)-8s:%(message)-8s'
    ...
    formatter1 = logging.Formatter('%(name)-8s:%(levelname)-8s%(message)s')
